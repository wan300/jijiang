...
| 微信支付 APIv3 密钥 | 服务器文件（700 权限）+ 环境变量指针 | 按微信要求 |
...
1. **请求签名**（RS256，用 API 证书）
2. **响应验签**（用微信平台证书）
3. **回调验签**（`Wechatpay-Signature` + 时间戳 + Nonce）
4. **报文解密**（APIv3 密钥 AES-256-GCM）

强烈建议使用官方 SDK `wechatpay-java`，避免自行实现。
...
public void handleNotify(HttpServletRequest req) {
    ...
    if (Math.abs(System.currentTimeMillis() / 1000 - Long.parseLong(timestamp)) > 300) {
        throw new BusinessException("回调时间戳超出 5 分钟");
    }
    String nonce = req.getHeader("Wechatpay-Nonce");
    String key = "pay:nonce:" + nonce;
    Boolean ok = redisHelper.setIfAbsent(key, "1", Duration.ofMinutes(10));
...
- **存库以元为单位 `DECIMAL(10,2)`**；传给微信支付以分为单位（Integer 或 String）
- **幂等校验**：`payment_record.out_trade_no` 唯一索引 + 状态前置判断
- **对账**：每日凌晨拉取微信支付账单，与本地流水比对（差异告警）
